...
export async function getCurrentUserFromCookie(request, env): Promise<User | null> {
...
// Return authenticated user, or null if no user
export async function getCurrentUserFromSessionId(sessionId, env): Promise<User | null> {
  if (sessionId) {
    const sessionUser = new AuthSession(env);
    const userId = await sessionUser.getUserBySession(sessionId);
    if (userId) {
